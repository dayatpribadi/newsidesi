(function() {
    ...
    // Add New Popup Toggle
    const addNew2PopupID = document.getElementById("add-new2-popup");
    if (addNew2PopupID) {
        var buttons = document.querySelectorAll("#add-new2-popup-toggle");
        buttons.forEach(function(button) {
            button.addEventListener("click", function() {
                // Toggle class on the div
                var divElement = document.getElementById("add-new2-popup");
                divElement.classList.toggle("active");
            });
        });
    }
    ...
    // Scrolling Long Content Modal
    const scrollingLonglessContentModal = document.getElementById("scrollingLonglessContentModal");
    if (scrollingLonglessContentModal) {
        var buttons = document.querySelectorAll("#scrollingLonglessContentModalToggle");
        buttons.forEach(function(button) {
            button.addEventListener("click", function() {
                // Toggle class on the div
                var divElement = document.getElementById("scrollingLonglessContentModal");
                divElement.classList.toggle("active");
            });
        });
    }
    ...
    // Quill Rich Text Editor
    const richTextEditor2= document.getElementById("richTextEditor2");
    if (richTextEditor2) {
        let quill = new Quill('#richTextEditor2', {
            theme: 'snow'
        });
    }

    // Trezo Tabs
    const trezoTabsMainID = document.getElementById("trezo-tabs");
    if (trezoTabsMainID) {
        document.addEventListener("DOMContentLoaded", function() {
    ...
    // Trezo Tabs
    document.addEventListener("DOMContentLoaded", function() {
        // Fungsi untuk menampilkan tab utama (Surat Masuk & Surat Keluar)
        function setupSidebarTabs() {
            ...
            sidebarButtons.forEach(button => {
                button.addEventListener("click", function() {
                    // Hapus semua 'active' dan sembunyikan semua tab content
                    sidebarButtons.forEach(btn => btn.classList.remove("bg-blue-100", "text-blue-600"));
                    tabContents.forEach(content => content.classList.add("hidden"));

                    // Tampilkan tab yang diklik
                    button.classList.add("bg-blue-100", "text-blue-600");
                    const contentId = button.getAttribute("data-tab-main");
                    document.getElementById(contentId).classList.remove("hidden");

                    // Pilih otomatis tab pertama di dalam tab utama
                    const firstInnerTab = document.querySelector(`#${contentId} .tab-button`);
                    if (firstInnerTab) {
                        firstInnerTab.click();
                    }
                });
            ...
            // Set Surat Masuk sebagai default aktif
            if (sidebarButtons.length > 0) {
                sidebarButtons[0].click();
            }
        }

        // Fungsi untuk menampilkan tab dalam Surat Masuk & Surat Keluar
        function setupInnerTabs() {
            ...
            allTabButtons.forEach(tab => {
                tab.addEventListener("click", function() {
                    ...
                    const siblingTabs = parentContent.querySelectorAll(".tab-button");
                    const siblingPanes = parentContent.querySelectorAll(".tab-pane");

                    // Hapus 'active' dari semua tab di dalam parent yang sama
                    siblingTabs.forEach(t => t.classList.remove("border-blue-500", "text-blue-500"));
                    siblingPanes.forEach(p => p.classList.add("hidden"));

                    // Tambahkan 'active' ke tab yang diklik dan tampilkan kontennya
        ...
        // Jalankan fungsi setup untuk sidebar dan inner tab
        setupSidebarTabs();
        setupInnerTabs();
    });
